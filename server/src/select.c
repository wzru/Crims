...
#define DEFINE_BINARY_ARI_OP(name, op) inline ExprNode* eval_##name(ExprNode *l, ExprNode *r, Record *rec) {\
...
#define DEFINE_BINARY_COM_OP(name, op) inline ExprNode* eval_##name(ExprNode *l, ExprNode *r, Record *rec) {\
        if(l==NULL || r==NULL) return &error_expr;\
        ExprNode *lhs = eval_expr(l, rec), *rhs = eval_expr(r, rec), *res = calloc(1, sizeof(ExprNode));\
        if(is_lazy(lhs) || is_lazy(rhs)) return &lazy_expr;\
